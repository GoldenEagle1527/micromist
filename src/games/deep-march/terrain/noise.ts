/**
 * Seeded 3D simplex noise (Stefan Gustavson's reference algorithm — the same
 * `snoise` family used by SebLague's `Noise.compute`, range ≈ [-1, 1]).
 * Pure / DOM-free so it runs in the mesher Web Worker and on the main thread.
 */
...
/** Hash an arbitrary seed string (or numeric string) to uint32. */
